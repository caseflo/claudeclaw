# PROGRESS.md — P0 Reliability Fixes

**Date:** 2026-04-21
**Repo:** `C:\Users\wendy\Desktop\claudeclaw-os`
**Audit commit:** `595b42e`

---

## Status: ALL 14 ITEMS COMPLETE

Verification: `npm run build` ✓, `npx tsc --noEmit` ✓, `npm run dev` (60s) — no unhandled rejection warnings, dashboard `/api/health` returns `{"ok":true,"db_ok":true}` ✓
...
|---|-------------|--------|--------|
| 1 | Add `@anthropic-ai/sdk` direct dep | **done** | `6ce230c` |
| 2 | Replace hardcoded baseURL with `ANTHROPIC_BASE_URL` env; throw if `ANTHROPIC_API_KEY` empty | **done** | `fd03178` |
| 3 | Remove `killExistingInstance()`; fail-fast PID check; add `unhandledRejection`/`uncaughtException` handlers; add `await bot.stop()` + `db.close()` in shutdown; add `process.send?.('ready')` | **done** | `4e9d718` |
| 4 | Create `ecosystem.config.cjs` with autorestart, memory limit, log paths | **done** | `70b3b4a` |
| 5 | Write `docs/SERVICE_SETUP.md` with pm2-installer, start, save, verification steps, log rotation commands | **done** | `4f14a22` |
| 6 | Add `/api/health` token-exempt endpoint; fix SSE listener leak via `c.req.raw.signal` abort listener | **done** | `ccce3ad` |
| 7 | PM2 log rotation — SKIP (commands added to SERVICE_SETUP.md during P0-5; no code change needed) | **skipped** | n/a |
| 8 | Hoist `statusMsg` above try in voice handler; guard in catch; add `bot.catch(err => log.error(...))` | **done** | `730da60` |
| 9 | Pass `{ signal }` into `client.messages.create` for proper abort support | **done** | `372137d` |
| 10 | Switch scheduler `setInterval` to self-rescheduling `setTimeout` to prevent overlapping ticks | **done** | `9fcb690` |
| 11 | Add `.playwright-cli/` to `.gitignore`; `git rm -r --cached .playwright-cli` | **done** | `cb78cd0` |
| 12 | Remove broken `status` and `mission` script entries from `package.json` | **done** | `3709867` |
| 13 | Replace `.env.example` with complete version from audit §4.4 | **done** | `9d15ec8` |
| 14 | Secret rotation — SKIP (Ramayne handling manually) | **skipped** | n/a |

---

## Deviations from Audit

- **P0-7:** The audit said "do NOT install yourself" for PM2 log rotation modules and listed commands to add to `SERVICE_SETUP.md`. The commands were added to `SERVICE_SETUP.md` during P0-5, so no separate commit was needed.
- **P0-2 + P0-3:** Committed separately per item (P0-2: agent.ts+config.ts, P0-3: index.ts) rather than together, which is clearer for git history.
- **P0-8:** Used `any` type for hoisted `statusMsg` because `grammy` v1.31 does not export a `Message` type directly. Type-unsafe but correct at runtime.

---

## What Was Not Touched

- P1 items (dual-master architecture)
- P2 items (cleanups, dead modules, memory fixes, etc.)
